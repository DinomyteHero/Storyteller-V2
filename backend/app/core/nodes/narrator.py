...
def make_narrator_node():
    ...
    def narrator_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        # Phase 5.1: Pass active_rumors and new_rumors from state into GameState
        # so the narrator prompt builder can weave them into the scene.
        if not gs.active_rumors and state.get("active_rumors"):
            gs.active_rumors = list(state.get("active_rumors") or [])
        ...
        final_text = corrected_text  # Apply any inline repairs from the validator
        if consistency_warnings and _turn_narrator._llm is not None:
            ...
            try:
                ...
                _corrected_retry, retry_warnings, _retry_repairs = _check_mechanic_consistency(retry_output.text, mechanic_result)
                if not retry_warnings:
                    final_text = _corrected_retry
                    output = retry_output
