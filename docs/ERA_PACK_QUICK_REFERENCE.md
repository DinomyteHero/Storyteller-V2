# Era Pack Quick Reference

This document consolidates all era pack documentation into a single quick reference guide.

## Overview

Era Packs are YAML-based content bundles that define the worldbuilding, NPCs, quests, factions, and tone for a specific Star Wars Legends era. Each pack contains 12 files that work together to create a cohesive narrative experience.

**Current Era Packs:**
- `_template` - Reference structure for creating new packs
- `rebellion` - Galactic Civil War (0 BBY - 4 ABY) - **Canonical reference pack**

**Removed Era Packs (can be regenerated):**
...
Each era pack lives in `/data/static/era_packs/{era_id}/` and contains **12 YAML files**:
...
## Quickstart: Using Era Packs

### 1. List Available Eras

```bash
curl http://localhost:8000/v2/debug/era-packs
```
...
### 5. Create Campaign with Era Pack
...
1. Copy the `_template` directory:
   ```bash
   cp -r data/static/era_packs/_template data/static/era_packs/new_era
   ```
...
4. Validate the pack:
   ```bash
   python scripts/validate_era_pack.py new_era
   ```

### Regenerating Deleted Era Packs

The following era packs were removed but can be regenerated:

```bash
# Using the LLM generation prompt
python -m backend.app.scripts.generate_era_pack \
  --era_id kotor \
  --time_period "3956 BBY" \
  --primary_conflict "Jedi vs. Sith" \
  --output data/static/era_packs/kotor
```

Or manually using the template in `/docs/era_pack_template.md`.
...
When enabled, NPCs are cast from era pack `npcs.yaml` templates rather than generated by LLM.

```python
ENABLE_BIBLE_CASTING = 1  # Use era pack deterministic casting
```

**Impact:**
- NPCs are pulled from era pack `npcs.yaml`
...
Era packs are loaded via `/backend/app/world/era_pack_loader.py`:

```python
from backend.app.world.era_pack_loader import get_era_pack

# Load an era pack
pack = get_era_pack("rebellion")
...
- `/docs/era_pack_template.md` - Comprehensive template with examples
- `/docs/era_pack_schema_reference.md` - Schema validation rules
- `/docs/era_pack_generation_prompt.md` - LLM prompt for automated generation
...
NPCs, factions, and locations are defined upfront in the "bible" (era pack YAML files) rather than procedurally generated. This ensures:
...
- Drop new YAML bundle into `/data/static/era_packs/{era_id}/`
...
### Era Pack Not Loading

1. Check file naming: all 12 files must be present
2. Validate YAML syntax: `yamllint data/static/era_packs/{era_id}/`
...
- `/data/static/era_packs/_template/` - Reference template
- `/data/static/era_packs/rebellion/` - Canonical example
- `/backend/app/world/era_pack_loader.py` - Loader implementation
- `/backend/app/world/era_pack_models.py` - Pydantic models
- `/docs/era_pack_template.md` - Full template documentation
